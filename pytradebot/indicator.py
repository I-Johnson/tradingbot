...
class Indicators():
    ...
    def rsi(self, period: int, method: str = 'wilders') -> pd.DataFrame:
        ...
        self._frame['down'] = self._price_groups['change_in_price'].transform(
            lambda x: np.where(x < 0, x.abs(), 0)
        )

        self._frame['ewna_up'] = self._price_groups['up_day'].transform(
            lambda x: x.ewm(span=period).mean()
        )

        self._frame['ewna_down'] = self._price_groups['down_day'].transform(
            lambda x: x.ewm(span=period).mean()
        )
    ...
    def refresh(self):
        ...
        for indicator in self._current_indicators:
            ...
            indicator_function = self._current_indicators[indicator]['cunc']
