...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str, enter_or_exit: str, price: float = 0.00, stop_limit_price: float = 0.00) -> dict:
        ...
        # Store important infor for later use
        if order_type == 'stop':
            self.stop_price = price
            
        elif order_type == 'stop-lmt':
            self.stop_price = stop_limit_price
        else: 
            self.stop_price = 0.0
    ...
    def instrunment(self, symbol: str, quantity: int, asset_type: str, sub_asset_type: str = None, order_leg_id: int = 0) -> dict:
        
        leg = self.order['orderLegCollection'][order_leg_id]

        leg['instrument']['symbol'] = symbol
        leg['instrunment']['assetType'] = asset_type
    ...
    def add_leg(self, order_leg_id: int, symbol: str, quantity: int, asset_type: str, sub_asset_type: str= None) -> List[dict]:
        # Define the leg
        leg = {}
        leg['instrunment']['symbol'] = symbol
        leg['instrunment']['assetType'] = asset_type
        leg['quantity'] = quantity
        
        if sub_asset_type:
            leg['instrunment']['subAssetType'] = sub_asset_type
